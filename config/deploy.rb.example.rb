...
server "gypsy", :app, :web, :db, primary: true

set (:user) { "USER" }
set (:application) { "gypsy" }
set (:deploy_to) { "/PATH/TO/#{application}" }
set (:nginx_server) { "IMAGE.EXAMPLE.COM" }
set (:goos) { "linux" }
set (:goarch) { "amd64" }
...
namespace :deploy do
  ...
  task :build, only: { primary: true } do
    run_locally("mkdir -p tmp; GOOS=#{goos} GOARCH=#{goarch} go build -o tmp/gypsy")
  end

  desc "Update the program binary on the host"
  task :update_code, only: [:app] do
    top.upload("tmp/gypsy", "#{deploy_to}/gypsy", {mode: "+x"})
  end
